...
# === MODIFIED: Import the enhanced ControlNet model ===
# Changed from warp_inpaint_model to warp_inpaint_model_controlnet to support DreamBooth + ControlNet
from models.warp_inpaint_model_controlnet import WarpInpaintModel
...
def run(config):
    # === MODIFIED: Enhanced setup with DreamBooth + ControlNet validation ===
    seed = config["seed"]
    if seed == -1:
    ...
    # === MODIFIED: Enhanced generation loop with progress tracking ===
...
if __name__ == "__main__":
    # === MODIFIED: Enhanced argument parser with better defaults ===
