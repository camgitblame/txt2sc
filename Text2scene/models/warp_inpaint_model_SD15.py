"""
WarpInpaintModel from the original SceneScape pipeline
===================================================================
This implementation is the original SceneScape pipeline's WarpInpaintModel,
which supports Stable Diffusion 1.5 and 2.0.

It uses the StableDiffusionInpaintPipeline with DDIMScheduler.

The model manually decodes latents using AutoencoderKL.
The first frame is generated by inpainting a white canvas using StableDiffusionInpaintPipeline.
It constructs a dynamic mesh using MiDaS depth predictions and projects it across time
to generate 3D-consistent video frames via inpainting.

The path to the Stable Diffusion model supported by SceneScape is defined
under `stable_diffusion_checkpoint` in the config file.
For example:
