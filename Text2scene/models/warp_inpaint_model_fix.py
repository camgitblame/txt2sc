...
- use_dreambooth_unet: Set to True for NON-INPAINTING DreamBooth models
- use_controlnet: Set to True to enable ControlNet structural guidance
- controlnet_model: ControlNet model for inpainting (default: lllyasviel/control_v11p_sd15_inpaint)
- controlnet_conditioning_scale: Strength of ControlNet conditioning (0.5-1.5, default: 1.0)
"""
...
class WarpInpaintModel(torch.nn.Module):
    def __init__(self, config):
        ...
        # CONTROLNET INTEGRATION: Load ControlNet for structural guidance if enabled
        self.use_controlnet = config.get("use_controlnet", False)
        if self.use_controlnet:
            # Load inpainting-specific ControlNet for better structural control
            self.controlnet = ControlNetModel.from_pretrained(
                config.get("controlnet_model", "lllyasviel/control_v11p_sd15_inpaint"),
                torch_dtype=torch.float16,  # Match main pipeline dtype
            )

        # === MODIFIED: HYBRID DREAMBOOTH SUPPORT ===
        # Support both inpainting and non-inpainting DreamBooth models
        use_dreambooth_unet = config.get("use_dreambooth_unet", False)

        if use_dreambooth_unet:
            # HYBRID APPROACH: For non-inpainting DreamBooth models
            # Load base SD1.5 inpainting pipeline, then replace UNet with DreamBooth UNet
            print(
                f"Loading NON-INPAINTING DreamBooth model: {config['stable_diffusion_checkpoint']}"
            )
            print(
                "Using hybrid approach: Base SD1.5 inpainting + DreamBooth UNet replacement"
            )

            if self.use_controlnet:
                # Base SD1.5 inpainting pipeline with ControlNet
                self.inpainting_pipeline = (
                    StableDiffusionControlNetInpaintPipeline.from_pretrained(
                        "runwayml/stable-diffusion-inpainting",  # Base SD1.5 inpainting
                ...
                # Base SD1.5 inpainting pipeline without ControlNet
                self.inpainting_pipeline = (
            ...
            # Replace UNet with DreamBooth UNet to preserve style
            from diffusers import UNet2DConditionModel

            print("Replacing UNet with DreamBooth UNet...")
            dreambooth_unet = UNet2DConditionModel.from_pretrained(
                config["stable_diffusion_checkpoint"],
                subfolder="unet",
                torch_dtype=torch.float16,
            )
            self.inpainting_pipeline.unet = dreambooth_unet
            print(
                "DreamBooth UNet replacement completed - style preserved with inpainting capability"
            )
        else:
            # For DreamBooth models already trained for inpainting
            print(
                f"Loading DreamBooth INPAINTING model: {config['stable_diffusion_checkpoint']}"
            )

            if self.use_controlnet:
                # For ControlNet, create pipeline with DreamBooth model components
                self.inpainting_pipeline = (
                    StableDiffusionControlNetInpaintPipeline.from_pretrained(
                        config[
                            "stable_diffusion_checkpoint"
                        ],  # DreamBooth inpainting model
                        controlnet=self.controlnet,
                        safety_checker=None,
                        torch_dtype=torch.float16,
                    )
                )
                print("DreamBooth ControlNet inpainting pipeline loaded successfully")
            else:
                # Standard inpainting with DreamBooth model
                self.inpainting_pipeline = (
                    StableDiffusionInpaintPipeline.from_pretrained(
                        config[
                            "stable_diffusion_checkpoint"
                ...
                print("DreamBooth inpainting pipeline loaded successfully")
        ...
        if self.config["use_xformers"]:
            try:
                self.inpainting_pipeline.set_use_memory_efficient_attention_xformers(
                    True
                )
                print("xformers memory efficient attention enabled")
            except ModuleNotFoundError:
                print("xformers not found, using default attention mechanism")
            except Exception as e:
                print(f"Failed to enable xformers: {e}")
                print("Continuing with default attention mechanism")

        # === For first frame generation ALWAYS use pure non-inpainting DreamBooth model for best style preservation ===
        print("Generating initial image with DreamBooth model...")
        mask_full = Image.new("RGB", (512, 512), "white")

        if self.config.get("use_dreambooth_unet", False):
            print(
                "Generating first frame with pure DreamBooth model for maximum style preservation..."
            )
            # Create dedicated DreamBooth text-to-image pipeline for first frame

            dreambooth_t2i_pipeline = StableDiffusionPipeline.from_pretrained(
                self.config["stable_diffusion_checkpoint"],  # DreamBooth model
                safety_checker=None,
                torch_dtype=torch.float16,  # Match main pipeline data type
            )
            dreambooth_t2i_pipeline.scheduler = DDIMScheduler.from_config(
                dreambooth_t2i_pipeline.scheduler.config
            )
            dreambooth_t2i_pipeline = dreambooth_t2i_pipeline.to(self.device)

            # Pure text-to-image generation with DreamBooth style
            image = dreambooth_t2i_pipeline(
            ...
            # Clean up memory
            del dreambooth_t2i_pipeline
            torch.cuda.empty_cache()
            print("First frame generated with pure DreamBooth style")
        elif self.use_controlnet:
            # For ControlNet, create control image for initial generation
            # Use simple conditioning for full image generation
            control_image = torch.zeros(1, 3, 512, 512).to(
                self.device
            )  # Empty control for initial gen
            image = self.inpainting_pipeline(
                ...
                controlnet_conditioning_scale=0.5,  # Lower conditioning for initial generation
                num_inference_steps=self.config["num_inpainting_steps"],
                guidance_scale=self.config["classifier_free_guidance_scale"],
            ).images[0]
        else:
            # Standard initial generation with DreamBooth inpainting model
            image = self.inpainting_pipeline(
                prompt=self.inpainting_prompt,
                negative_prompt=self.config["negative_inpainting_prompt"],
                image=mask_full,
        ...
        # === ENHANCED VAE LOADING ===
        # Load VAE from DreamBooth model if using hybrid approach, otherwise use model's VAE
        use_dreambooth_unet = config.get("use_dreambooth_unet", False)
        print("Loading VAE...")

        try:
            if use_dreambooth_unet:
                # For hybrid approach, use DreamBooth VAE for consistency
                print("Loading VAE from DreamBooth model for hybrid approach...")
                self.vae = AutoencoderKL.from_pretrained(
                    config["stable_diffusion_checkpoint"],
                    subfolder="vae",
                    torch_dtype=torch.float16,
                ).to(self.device)
                print("DreamBooth VAE loaded successfully")
            else:
                # For direct inpainting models, use the model's VAE
                print("Loading VAE from inpainting model...")
                self.vae = AutoencoderKL.from_pretrained(
                    config["stable_diffusion_checkpoint"],
                    subfolder="vae",
                    torch_dtype=torch.float16,
            ...
            # Fallback to SD1.5 VAE if not found in model
            print(f"VAE loading failed ({e}), using SD1.5 VAE as fallback")
            self.vae = AutoencoderKL.from_pretrained(
        ...
        # >>> ADD WARPING PATCH <<<
        if attach_fast_fixes and self.config.get("enable_warp_fix", True):
            attach_fast_fixes(
                self, config_overrides=self.config.get("warp_fix_overrides", {})
            )
    ...
    def get_depth(self, image):
        # === Data type compatibility for MiDaS depth model ===
        # Ensure image is in float32 for MiDaS model to prevent Half/float type mismatch
        image_float32 = image.float()  # Convert to float32 for depth estimation
        ...
        depth = depth.to(dtype=image.dtype)
        disparity = disparity.to(dtype=image.dtype)
    ...
    def warp_mesh(self, epoch=None, camera=None):
        ...
        if self.config["fix_floating_artifacts"]:
            ...
            # Instead of setting to white, preserve neighboring colors or use neutral gray to reduces harsh white artifacts in unmapped regions
            masked_regions = inpaint_mask.bool().repeat(1, 3, 1, 1)
            # Use average color of surrounding area instead of pure white
            if masked_regions.any():
                # Dilate to get neighboring pixels
                from kornia.morphology import dilation

                kernel = torch.ones((5, 5), device=self.device)
                dilated_mask = dilation(inpaint_mask, kernel)
                boundary_mask = (dilated_mask - inpaint_mask) > 0

                if boundary_mask.any():
                    # Get average color of boundary pixels
                    boundary_colors = warped_image * boundary_mask.repeat(1, 3, 1, 1)
                    avg_color = boundary_colors.sum() / (boundary_mask.sum() * 3 + 1e-8)
                    warped_image[masked_regions] = avg_color.clamp(0, 1)
                else:
                    # Fallback to neutral gray instead of white
                    warped_image[masked_regions] = 0.5
        ...
        if self.config["antialiasing_factor"] > 1:
            ...
            mask = warped_depth != -1
            # Use neutral gray (0.5) instead of white (1) for unmapped regions
            warped_image = self.masked_downsample(warped_image, mask, put_in_mask=0.5)
    ...
    def make_inpaint_condition(self, image, image_mask):
        """
        CONTROLNET CONDITIONING: Create control image for ControlNet inpainting
        Prepare the control condition by masking pixels and dilating the mask
        to provide structural guidance to the ControlNet model
        """
        ...
        # Dilate mask to include more context around masked regions
        kernel = np.ones((5, 5), np.uint8)
        image_mask = cv2.dilate(image_mask, kernel, iterations=3)
        # Set masked pixels to -1.0 as required by ControlNet inpainting
    ...
    def inpaint(self, warped_image, inpaint_mask):
        if self.use_controlnet:
            ...
            inpainted_images = self.inpainting_pipeline(
                ...
                mask_image=mask_pil,
                control_image=control_image,  # ControlNet conditioning
            ...
            # STANDARD INPAINTING: Use DreamBooth inpainting pipeline directly
            inpainted_images = self.inpainting_pipeline(
                prompt=self.inpainting_prompt,
                negative_prompt=self.config["negative_inpainting_prompt"],
                image=ToPILImage()(warped_image[0]),
    ...
    def decode_latents(self, latents):
        # === Data type compatibility for hybrid DreamBooth approach ===
        # Ensure latents match VAE data type to prevent float/half mismatch errors
